...
class Application:
    ...
    def start_application(self):
        ...
        if self.option == "1" or self.option == "2":
            first_generation_matrix = self._generate_and_print_initial_generation(
                self.option
            )
            ...
            ) = self._generate_and_print_second_generation(
                first_generation_matrix, NUMBER_OF_GENERATION
            )
            while NUMBER_OF_GENERATION <= 10:
                (
                    next_generation_matrix,
                    NUMBER_OF_GENERATION,
                ) = self._generate_and_print_next_generation(
                    next_generation_matrix, NUMBER_OF_GENERATION
                )
            InputOutput(self.option, None).end_summary()
        elif self.option == "5":
            first_generation_matrix = self._generate_and_print_initial_generation(
    ...
    def _generate_and_print_second_generation(
        self, first_generation_matrix, number_of_generation
    ):
        (
            next_generation_matrix,
            number_of_generation,
        ) = self.generate_second_generation_matrix(
            number_of_generation, first_generation_matrix
        )
    ...
    def _generate_and_print_next_generation(
        self, first_generation_matrix, number_of_generation=None
    ):
        if number_of_generation is None:
            ...
            self.print_next_generation_matrix(
                number_of_generation, next_generation_matrix
            )
        else:
            next_generation = NextGeneration(first_generation_matrix)
            next_generation_matrix = next_generation.next_generation_of()
            self.print_intermediate_generation_matrix(
                next_generation_matrix, number_of_generation
            )
            number_of_generation += 1
            return next_generation_matrix, number_of_generation
    ...
    def print_next_generation_matrix(
        self, number_of_generation, next_generation_matrix
    ):
    ...
    def generate_second_generation_matrix(
        self, number_of_generation, first_generation_matrix
    ):
        next_generation = NextGeneration(first_generation_matrix)
        next_generation_matrix = next_generation.next_generation_of()
        number_of_generation += 1
    ...
    def print_intermediate_generation_matrix(
        self, next_generation_matrix, number_of_generation=None
    ):
        input_output = InputOutput(None, next_generation_matrix)
        if number_of_generation:
            sleep(0.5)
